...
export function buildForwardShipmentPayload(order, options = {}) {
    ...
    // Build order items — discount% is converted to flat amount for Shiprocket
    ...
    // Order-level totals calculated from items (pre-discount)
    const itemsSubTotal = orderItems.reduce((sum, item) => sum + (item.selling_price * item.units), 0)
    const itemsTotalDiscount = orderItems.reduce((sum, item) => sum + (item.discount * item.units), 0)
    const promoDiscount = Math.round(discount || 0)
    ...
    const payload = {
        ...
        transaction_charges: Math.round(order.tax || 0),
        total_discount: itemsTotalDiscount + promoDiscount,
