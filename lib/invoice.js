...
// 'cod' → 'COD'; any online method (card/upi/netbanking/wallet) → 'Online'
function fmtPaymentMethod(method) {
    return method === 'cod' ? 'COD' : 'Online'
}

// For COD the payment is always pending until the delivery partner confirms receipt.
// For online payments map the gateway status to a human-readable label.
function fmtPaymentStatus(method, status) {
    if (method === 'cod') {
        return status === 'completed' ? 'Collected' : 'Pending — Collected on Delivery'
    }
    const labels = { completed: 'Successful', failed: 'Failed', refunded: 'Refunded', pending: 'Pending' }
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const detailRows = [
                ...
                ['Payment',   fmtPaymentMethod(order.payment?.method)],
                ['Status',    fmtPaymentStatus(order.payment?.method, order.payment?.status)],
            ]
            ...
            tY += 13
            doc.font('Helvetica').fontSize(8.5).fillColor(C.muted)
               .text(`Method: ${fmtPaymentMethod(order.payment?.method)}   |   Status: ${fmtPaymentStatus(order.payment?.method, order.payment?.status)}`, M, tY, { lineBreak: false })
            if (order.payment?.transactionId) {
                tY += 13
